    ...
    const apiData = () => {
        ...
        return fetch(url)
            ...
            .then(data => {
                ...
                //menuDataFromApi = data;
                setmenuDataFromApi(data);

                // console.log(menuDataFromApi);
    ...
    const getCategories = () => {
        ...
        for (let i = 0; i < menuDataFromApi.length; i++) {
            if (!cat.includes(menuDataFromApi[i].Category)) {
                ...
                cat.push(menuDataFromApi[i].Category);
                //console.log(menuDataFromApi[i].Category);
                catImg.push(menuDataFromApi[i].Image);
            }
        }
        setMenuCategories(cat);
        setMenuCatImg(catImg);
    }
